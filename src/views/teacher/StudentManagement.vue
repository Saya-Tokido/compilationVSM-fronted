        ...
        <table class="student-table">
            <thead>
                <tr>
                    ...
                    <th>
                        客观题成绩
                        <span class="sort-icon" :class="{'active': filters.objAsc!== null}" @click="toggleSort('objAsc')">
                            {{ filters.objAsc === 1? '↑' : (filters.objAsc === 0? '↓' : '-') }}
                        </span>
                    </th>
                    <th>
                        词法分析题成绩
                        <span class="sort-icon" :class="{'active': filters.lexerAsc!== null}" @click="toggleSort('lexerAsc')">
                            {{ filters.lexerAsc === 1? '↑' : (filters.lexerAsc === 0? '↓' : '-') }}
                        </span>
                    </th>
                </tr>
            </thead>
...
export default {
    setup(props,{emit}) {
        ...
        const fetchStudents = async () => {
            ...
            try {
                const response = await axios.post(`${apiUrl}/teacher/student-manager/basic-info`, requestData, { headers });
        ...
        const toggleSort = (field) => {
            if (field === 'objAsc') {
                if (filters.value.objAsc === null) {
                    filters.value.objAsc = 1;
                } else if (filters.value.objAsc === 1) {
                    filters.value.objAsc = 0;
                } else {
                    filters.value.objAsc = null;
                }
                filters.value.lexerAsc = null;
            } else if (field === 'lexerAsc') {
                if (filters.value.lexerAsc === null) {
                    filters.value.lexerAsc = 1;
                } else if (filters.value.lexerAsc === 1) {
                    filters.value.lexerAsc = 0;
                } else {
                    filters.value.lexerAsc = null;
                }
                filters.value.objAsc = null;
